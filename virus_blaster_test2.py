                        ...
                        for attempt in range(retries):
                            try:
                                with Entrez.efetch(db="nucleotide",
                                                   ...
                                                   seq_stop=hsp.sbjct_end) as fetch_handle:
                                    fetch_record = SeqIO.read(fetch_handle, "genbank")
                            ...
                            if fetch_record:
                                hsp_sbjct_start = min(hsp.sbjct_start, hsp.sbjct_end)
                                hsp_sbjct_end = max(hsp.sbjct_start, hsp.sbjct_end)
                                ...
                                if len(header_parts) >= 5:
                                    ...
                                    start_location = hsp_sbjct_start
                                    end_location = hsp_sbjct_end
                                    strand = '+' if hsp.frame[1] > 0 else '-'
                                    formatted_accession_id = fetch_record.id.replace('.', '_')
                                    ...
                                    # Save the subject sequence
                                    subject_record = SeqRecord(
                                        Seq(fetch_record.seq),
                                        id=alignment.hit_id,
                                        description=alignment.hit_def
                                    )
                                    with open(output_file_path, "w") as output_handle:
                                        SeqIO.write(subject_record, output_handle, "fasta")
                                        logging.info(f'Saved subject sequence to {output_file_path}')
