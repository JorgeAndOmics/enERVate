...
def merge_overlapping_sequences(sequences):
    ...
    try:
        ...
        for (accession_id, strand), seqs in sequences_by_accession_and_strand.items():
            ...
            current_seq = seqs[0]
            current_headers = [current_seq["header"]]
            for next_seq in seqs[1:]:
                if next_seq["start"] <= current_seq["end"]:
                    logging.info("Found sequences to merge")
                    ...
                    current_headers.append(next_seq["header"])
                    logging.debug(f"Merging sequences: {current_seq['accession_id']}:{current_seq['start']}-"
                                  f"{current_seq['end']} and {next_seq['accession_id']}:{next_seq['start']}-"
                                  f"{next_seq['end']}")
                else:
                    if accession_id not in merged_sequences_dict:
                        merged_sequences_dict[accession_id] = []
                    merged_sequences_dict[accession_id].append((current_seq, current_headers))
    ...
    merged_sequences = [(seq, headers) for accession_list in merged_sequences_dict.values() for seq, headers in accession_list]
    return merged_sequences

def verify_merged_sequences(merged_sequences):
    """Verifies the merged sequences for correct length."""
    for seq, headers in merged_sequences:
        expected_length = seq['end'] - seq['start'] + 1
        if len(seq['sequence']) != expected_length:
            logging.warning(f"Sequence length mismatch for {seq['accession_id']}: "
...
def write_merged_sequences(original_dir, target_dir, merged_sequences, all_sequences):
    ...
    for seq, headers in merged_sequences:
        try:
            ...
            # Use the start of the first sequence and the end of the last sequence
            combined_start = seq['start']
            combined_end = seq['end']
            if len(headers) > 1:
            ...
            full_header = "|".join(headers)
            record = SeqRecord(Seq(seq["sequence"]), id=full_header, description="")
            output_filepath = os.path.join(target_dir, header)
            with open(output_filepath, "w") as output_file:
                SeqIO.write(record, output_file, "fasta")
            merged_accession_ids.add(seq["accession_id"])
            logging.debug(f"Wrote merged sequence to {output_filepath}")
        except Exception as e:
...
def process_directory(args):
    ...
    try:
        ...
        logging.debug(f"Finished processing for directory {original_dir}")
    except Exception as e:
